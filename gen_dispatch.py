...
with open(THIS / 'dispatch_all.h', 'w') as h:
...
with open(THIS / 'dispatch_all.c', 'w') as c:
...
existing_emu = set()
try:
    with open(THIS / 'ff4_helpers.c') as f:
        for line in f:
...
if needed_emu:
    with open(THIS / 'dispatch_all.c', 'a') as c:
        c.write('\n\n/* ---------------------------------------------------------\n')
