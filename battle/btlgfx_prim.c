...
void Mult8_btlgfx_c(Snes *snes) {
     ...
     * and clobber $26/$28 in WRAM.  Using stale values for the product would
     * write the wrong DMA parameters into $1826-$182B, causing TfrMenuTilesUpdate
     * to DMA the command-window tilemap to the wrong VRAM address (f20 artefact). */
    ...
    /* Use saved inputs — $26/$28 in WRAM may have been clobbered by an NMI above. */
    uint16_t product = (uint16_t)multiplicand * (uint16_t)mult;
    snes->ram[0x2A] = (uint8_t)(product & 0xFF);
    snes->ram[0x2B] = (uint8_t)(product >> 8);
    /* Simulate 8 ROR on $28 (with carry propagation) to match the 65816 loop side-effect.
     * Use saved mult — $28 in WRAM may carry the inner NMI call's rotated value. */
    {
        uint8_t b = mult;
        uint8_t c = snes->cpu->c;
        for (int i = 0; i < 8; i++) {
            uint8_t nc = b & 1;
            b = (b >> 1) | (c << 7);
            c = nc;
